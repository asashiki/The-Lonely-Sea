...
const OPTION_COPY = Object.freeze({
  "EN-US": Object.freeze({
    调试: "Debug", 试听: "Play", 停止: "Stop", 当前立绘: "Current artwork", 查看: "View",
  ...
  "JA-JP": Object.freeze({
    调试: "デバッグ", 试听: "試聴", 停止: "停止", 当前立绘: "現在の立ち絵", 查看: "表示",
...
export function initOptions({ onReplayOpening = () => {}, onResetExperience = () => {} } = {}) {
  ...
  function stopVoiceTest() {
    ...
    if (voiceTestButton) voiceTestButton.textContent = optionCopy("试听");
  }
  voiceTestButton?.addEventListener("click", async () => {
    ...
    try {
      await voiceTest.play();
      voiceTestButton.textContent = optionCopy("停止");
    } catch { stopVoiceTest(); }
  });
  ...
  function selectPanel(owner, id, { focus = false } = {}) {
    ...
    soundLaboratory?.setActive(owner === "debug" && id === "sound-lab");
    if (id !== "alice-test") stopVoiceTest();
