...
async function checkSystemAtmosphere(browser) {
  ...
  await page.getByRole("tab", { name: "海景", exact: true }).click();
  await page.getByRole("button", { name: "系统页延续天气", exact: true }).click();
  ...
  await page.getByRole("radio", { name: "盖过 UI", exact: true }).click();
