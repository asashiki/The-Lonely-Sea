(() => {
  ...
  const preloadedDifferentials = new Set();
  ...
  const preloadFigureDifferentials = (animation) => {
    ...
    for (const path of new Set(paths)) {
      if (preloadedDifferentials.has(path)) continue;
      preloadedDifferentials.add(path);
  ...
  const installNativeUiAdapter = () => {
    ...
    const createRuntimeMenu = () => {
      ...
      runtimeMenu.addEventListener("click", (event) => {
        ...
        if (action === "log") activateNativeControl(["回想", "Backlog", "Log", "ログ"]);
